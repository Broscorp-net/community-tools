...
public class ClassroomServiceImpl implements ClassroomService {
   ...
   * for repositories in the traineeship organization.
   * This method retrieves the repositories from the organization,
   * checks their workflows' statuses, and processes pull requests accordingly.
   * <p>
   * If a task is not assigned, a link to the pull request is sent
   * to the 'ready-for-review' channel.
   * If at least one reviewer has subscribed, send them an email notification,
   * provided that the build has passed and the 'ready for review' tag is set.
   * If the build is unsuccessful and more than a day has passed, an auto-comment
   * is added indicating that tasks with failed builds will not be reviewed.
   * </p>
   *
  ...
  public void handleNotifications() throws IOException {
    ...
    for (Map.Entry<String, GHRepository> entry : repositories.entrySet()) {
      ...
      GHWorkflowRun lastWorkflow = getWorkflowRun(repository);
      GHWorkflowRun.Conclusion lastWorkflowConclusion =
              lastWorkflow == null
                      ? GHWorkflowRun.Conclusion.FAILURE : lastWorkflow.getConclusion();
